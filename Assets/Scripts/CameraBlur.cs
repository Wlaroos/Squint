...
public class CameraBlur : MonoBehaviour
{
	...
	private void Update()
	{
		...
		{
			StopAllCoroutines();
			StartCoroutine(Focus(_durationInSeconds));
		}
		
		if(Input.GetKeyUp(KeyCode.Z))
		{
			StopAllCoroutines();
			StartCoroutine(UnFocus(_durationInSeconds));
		}
	...
	private IEnumerator Focus(float blurTime)
	{
		float timeCount = 0;
	...
	private IEnumerator UnFocus(float blurTime)
	{
		float timeCount = 0;
